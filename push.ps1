...
# Separate code and media/font files to handle them with different batch sizes
$codeFiles = @()
$mediaFiles = @()
foreach ($file in $allFiles) {
    if ($file -match '\.(png|jpg|jpeg|gif|ico|pdf|zip|tar|gz|exe|apk|appimage|deb|rpm|ttf|webp)$') {
        $mediaFiles += $file
    } else {
        $codeFiles += $file
    }
...
Write-Host "Found $($codeFiles.Count) code files and $($mediaFiles.Count) media/binary/font files to push." -ForegroundColor Yellow
...
# --- Phase B: Push Media/Binary/Font Files One-by-One ---
if ($mediaFiles.Count -gt 0) {
    Write-Host ""
    Write-Host "--- PUSHING MEDIA/BINARY/FONT FILES (individually) ---" -ForegroundColor Yellow
    ...
    foreach ($file in $mediaFiles) {
        ...
        try {
            ...
            if ($LASTEXITCODE -ne 0) {
                Write-Host ""
                Write-Host "WARNING: Git push failed on media file: $file. Reverting commit and skipping..." -ForegroundColor Yellow
                git reset --mixed HEAD~1
            } else {
                Write-Host "SUCCESS: Pushed $file" -ForegroundColor Green
            }
        } catch {
            Write-Host ""
            Write-Host "WARNING: Git operations failed for $file. Reverting and skipping..." -ForegroundColor Yellow
            try { git reset --mixed HEAD~1 } catch {}
        }
        $fileNum++
        Start-Sleep -Seconds 1
    }
...
Write-Host ""
Write-Host "SUCCESS: All files have been successfully processed and pushed to GitHub!" -ForegroundColor Green
